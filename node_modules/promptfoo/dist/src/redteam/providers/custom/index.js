...
class CustomProvider {
    constructor(config) {
        ...
        logger_1.default.debug(`[Custom] CustomProvider initialized with config: ${JSON.stringify(config)}`);
    }
    ...
    async callApi(prompt, context, options) {
        logger_1.default.debug(`[Custom] callApi context: ${(0, json_1.safeJsonStringify)(context)}`);
    ...
    async runAttack({ prompt, filters, vars, provider, context, options, test, }) {
        ...
        while (roundNum < this.maxTurns) {
            try {
                ...
                if (attackTokenUsage) {
                    (0, tokenUsageUtils_1.accumulateTokenUsage)(totalTokenUsage, attackTokenUsage);
                }
                if (!attackPrompt) {
                ...
                (0, tokenUsageUtils_1.accumulateResponseTokenUsage)(totalTokenUsage, unblockingResult);
        ...
        `;
                ...
                if (test && assertToUse) {
                    ...
                    if (grader) {
                        ...
                        (0, tokenUsageUtils_1.accumulateGraderTokenUsage)(totalTokenUsage, grade);
                    }
                }
                ...
                if (evalTokenUsage) {
                    (0, tokenUsageUtils_1.accumulateTokenUsage)(totalTokenUsage, evalTokenUsage);
                }
    ...
    async getAttackPrompt(roundNum, evalFlag, lastResponse, lastFeedback, objectiveScore) {
        ...
        if (!response.output) {
            logger_1.default.debug(`[Custom] No output from redteam provider: ${JSON.stringify(response)}`);
            return {
    ...
    async sendPrompt(attackPrompt, originalPrompt, vars, filters, provider, roundNum, context, options) {
        ...
        logger_1.default.debug(`[Custom] Target response: ${JSON.stringify(targetResponse)}`);
        if (targetResponse.error) {
            throw new Error(`[Custom] Target returned an error: ${targetResponse.error}`);
        }
        (0, invariant_1.default)(targetResponse.output, '[Custom] Target did not return an output');
    ...
    async getRefusalScore(attackPrompt, lastResponse) {
        ...
        logger_1.default.debug(`[Custom] Refusal score parsed response: ${JSON.stringify(parsed)}`);
    ...
    async getEvalScore(lastResponse) {
        ...
        const parsed = typeof evalResponse.output === 'string'
            ...
            : evalResponse.output;
        logger_1.default.debug(`[Custom] Eval score parsed response: ${JSON.stringify(parsed)}`);
