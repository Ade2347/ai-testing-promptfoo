...
async function runRedteamConversation({ prompt, filters, vars, redteamProvider, targetProvider, injectVar, context, options, test, }) {
    ...
    for (let i = 0; i < numIterations; i++) {
        try {
            ...
            let isOnTopic = false;
            try {
                const parsed = (0, json_1.extractFirstJsonObject)(isOnTopicResp.output);
                isOnTopic = parsed.onTopic;
                logger_1.default.debug(`Iteration ${i + 1}: Prompt on-topic check: ${isOnTopic}`);
            }
            catch (err) {
                logger_1.default.warn(`Iteration ${i + 1}: Failed to parse on-topic response: ${err}`);
                continue;
            }
            ...
            if (url && url.length > 0) {
                try {
                    ...
                    (0, tokenUsageUtils_1.accumulateResponseTokenUsage)(totalTokenUsage, visionResponse);
