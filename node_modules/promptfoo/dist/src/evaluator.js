...
class ProgressBarManager {
    constructor(isWebUI) {
        ...
        // Track completion
        this.serialCompleted = 0;
        this.concurrentCompleted = 0;
        this.comparisonCompleted = 0;
        // Map original indices to execution context
        this.indexToContext = new Map();
    ...
    async initialize(runEvalOptions, concurrency, compareRowsCount) {
        ...
        this.multibar = new cli_progress_1.default.MultiBar({
            format: '{phase} [{bar}] {percentage}% | {value}/{total} | {status} | {provider} "{prompt}" {vars}',
            hideCursor: true,
            gracefulExit: true,
     ...
     */
    updateProgress(index, evalStep, phase = 'concurrent') {
        if (this.isWebUI || !evalStep) {
            return;
        }
        const context = this.indexToContext.get(index);
        if (!context) {
            logger_1.default.warn(`No context found for index ${index}`);
            return;
        }
        ...
        switch (context.phase) {
            ...
            case 'concurrent':
                ...
                if (context.barIndex >= 0 && context.barIndex < this.concurrentBars.length) {
                    ...
                    logger_1.default.warn(`Invalid bar index ${context.barIndex} for concurrent progress update`);
                }
                break;
        }
    }
    ...
    updateComparisonProgress(prompt) {
        ...
        if (this.comparisonCompleted >= this.comparisonCount) {
            logger_1.default.warn(`Comparison progress already at maximum (${this.comparisonCompleted}/${this.comparisonCount})`);
            return;
        }
     ...
     */
    createComparisonBar(comparisonCount) {
...
class Evaluator {
    ...
    async _runEvaluation() {
        ...
        const modelGradedCount = Array.from(assertionTypes).filter((type) => assertions_1.MODEL_GRADED_ASSERTION_TYPES.has(type)).length;
        ...
        const timeoutOccurred = evalTimedOut ||
            this.evalRecord.results.some((r) => r.failureReason === types_1.ResultFailureReason.ERROR && r.error?.includes('timed out'));
